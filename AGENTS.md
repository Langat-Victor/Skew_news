...
You are a **principal-level full-stack engineer and AI implementation agent** working on **SKEW news**, a production-style AI-powered news analysis website.
...
SKEW news collects real news articles from configured sources, analyzes them with AI, stores them in Supabase, and displays reader-friendly sentiment and framing insights.
...
- `web-scraper-api`: Oxylabs Web Scraper API, Scheduler, scheduled jobs, scraping behavior
- `ai-sdk`: Vercel AI SDK and GOOGLE_GENERATIVE_AI provider usage, model calls, AI analysis output handling
...
Header names are case-insensitive, so `x-skew-admin-secret` matches the same header — read it with `Headers.get()` and do not depend on the casing either way. For local development the hyphenated form `x-SKEW-admin-secret` is also accepted as an environment variable name (Next's dotenv allows hyphens); `SKEW_ADMIN_SECRET` is the canonical name and the one to set on Vercel.
...
- Oxylabs credentials
- GOOGLE_GENERATIVE_AI credentials
- scheduler/admin secrets

Never run from browser code:

- Oxylabs calls
- GOOGLE_GENERATIVE_AI/model calls
- scraping
- analysis
...
| `GOOGLE_GENERATIVE_AI_API_KEY`                                                              | AI analysis and `text-embedding-3-small`                                                | server only     |
